...
@onready var anim_button_container = find_child("AnimationButtons")
@onready var play_pause_button = find_child("PlayPause")
@onready var add_animation_button = find_child("AddAnimation") # for dragging
...
func add_animation_stripe():
	var anim_stripe = AnimationStripe.instantiate()
	anim_button_container.add_child(anim_stripe)
	anim_stripe.connect("on_frame_pressed",Callable(self,"_on_frame_pressed"))
	anim_stripe.connect("on_add_frame_pressed",Callable(self,"_on_add_frame_pressed"))
	anim_stripe.connect("on_animation_pressed",Callable(self,"_on_animation_pressed"))
	anim_stripe.connect("on_move_down_pressed",Callable(self,"move_animation_down"))
	anim_stripe.connect("on_move_up_pressed",Callable(self,"move_animation_up"))
	anim_stripe.connect("on_moved",Callable(self,"_on_moved"))
	anim_stripe.connect("on_animation_duplicated",Callable(self,"_on_animation_duplicated"))
	anim_stripe.connect("on_animation_selected",Callable(self,"_on_animation_selected"))
	anim_stripe.connect("on_deleted",Callable(self,"_on_deleted"))
