...
@onready var frame_button_container = find_child("FrameContainer")
@onready var anim_button = find_child("AnimationButton")
var frames = []
...
func _input(event):
		...
		if event is InputEventMouseButton:
			if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
				if not dragging and get_global_rect().has_point(get_global_mouse_position()):
					...
					mouse_offset = mp - global_position
	...
	var mp = get_global_mouse_position().y - get_parent().global_position.y
	var y_index = mp / (size.y + get_parent().get("custom_constants/separation"))
...
func add_new_frame_button():
	var frame_button = FrameButton.instantiate()
	frame_button_container.add_child(frame_button)
	frame_button.connect("on_frame_pressed",Callable(self,"_on_frame_pressed"))
	return frame_button
