...
func _enter_tree():
	...
	for child in get_children():
		if child.is_connected("pressed",Callable(self,"change_color_to")):
			return
		child.connect("pressed",Callable(self,"change_color_to").bind(child.get("custom_styles/normal").bg_color))
