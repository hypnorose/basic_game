...
#terms
#global cell - a cell that has a global grid position checked the canvas
#local cell - a cell that has a local grid position in a chunk region checked the canvas
...
#			layer_data.remove_at(pixel)
#			#update_chunk_region_from_cell(x, y)
#			#TOOD: If pixel exists in temp_pool_pixels then remove_at it
...
func set_pixel_cell(x, y, color):
	...
	for pixel in layer.data:
		...
		if pixel[0] == x and pixel[1] == y:
			...
			#so we remove_at it from the layer data
			if color == Color(0, 0, 0, 0):
				layer.data.remove_at(index)
			else:
				pixel[2] = color
