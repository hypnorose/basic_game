...
var selected_color = Color(1, 1, 1, 1) :
	get:
		return selected_color # TODOConverter40 Non existent get function 
	set(mod_value):
		mod_value  # TODOConverter40 Copy here content of set_selected_color
...
func _handle_shortcuts(event: InputEvent):
	...
	elif event.scancode == KEY_CTRL:
		ctrl_pressed = event.pressed
	
	if not event.pressed:
		return
	
	# delegate shortcuts depending checked modifiers
	if shift_pressed:
		...
		if shortcuts[KEY_CTRL].has(event.scancode):
			_handle_shortcut(shortcuts[KEY_CTRL][event.scancode])
			accept_event()
			return
...
func delete_frame(anim_idx, frame_idx):
	if animations[anim_idx].frames.size() <= 1:
		return
	animations[anim_idx].frames.remove_at(frame_idx)
